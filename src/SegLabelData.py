...
class SegLabel(object):
  ...
  def __init__(self, alphas, betas, map_DNNdet_reindex, map_DNNclass_reindex, person_dic, dets_prob_dic, dets_pos):
    self.num_detections = len(dets_prob_dic.keys())
    self.num_classes = alphas.shape[1]
    self.max_num_persons = None
    if person_dic is not None:
    ...
    self.dets_pos = dets_pos # pixel coordinate of each detection. dets_pos[i] = [x,y], len(dets_pos) = num_detections
    self.person_dic = person_dic 
    """ 
    person_dic == None if we have no annotations for the related image
    person_dic = { 
          0 : { # person 0
              joint_type_1 : {det_idx1 : distance_to_joint_of_same_person, 
              det_idx2 : distance_to_joint_of_same_person}, 
